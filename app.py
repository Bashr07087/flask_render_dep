...
def upload():
    ...
    if request.method == 'POST':
        ...
        if not file or not title or not artist:
            return render_template('upload.html', error='All fields required')

        if not allowed_audio(file.filename):
            return render_template('upload.html', error='Invalid audio format')

        audio_fn = save_file(file)
        cover_fn = save_file(cover) if cover and allowed_image(cover.filename) else None
...
def init_db():
    ...
    if not User.query.filter_by(username='owner').first():
        ...
        owner.set_password(os.environ.get("OWNER_PASSWORD", "ChangeThisPassword123!"))
